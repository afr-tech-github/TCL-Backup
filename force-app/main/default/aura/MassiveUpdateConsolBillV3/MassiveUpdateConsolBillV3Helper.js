({
    ...
    initNewData: function (cmp) {
        ...
        var getNewBillings = cmp.get("c.getSubBillings");
    ...
    createComponent: function (cloneCmp, billing, isFirstRecord) {
        ...
        $A.createComponent(
            "c:MassiveUpdateConsolBillItemV3",
            {
                id: isFirstRecord ? '' : billing.Id,
                'aura:id': 'massiveUpdateConsolBillItemV3',
                isFirstRecord,
                // isSelling,
                parent: cloneCmp,
            ...
            (massiveUpdateConsolBillItemV3, status, errorMessage) => {
                if (status === "SUCCESS") {
                    var body = cloneCmp.get("v.body");
                    body.push(massiveUpdateConsolBillItemV3);
    ...
    onToggleSelect: function (cmp, isSelected) {
        var cmps = cmp.find('massiveUpdateConsolBillItemV3'), formattedCmps = [];
        var allRecords = [];
    ...
    onSave: function (cmp, changedFields) {
        ...
        var cmps = cmp.find('massiveUpdateConsolBillItemV3'), formattedCmps = [];
        var allRecords = [];
        console.log('111 :'+cmps.length);
        if (!cmps) return;
        if ($A.util.isArray(cmps)) {
            for (var i = 0; i < cmps.length; i++) {
                formattedCmps.push(cmps[i]);
            }
        } else {
           (formattedCmps = [cmps]);
        }
        console.log('formattedCmps :'+formattedCmps);
        for (var i = 0; i < formattedCmps.length; i++) {
